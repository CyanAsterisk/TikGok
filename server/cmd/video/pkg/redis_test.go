...
func TestVideoLifecycle(t *testing.T) {
	...
	cleanUpFunc, client, err := test.RunWithRedisInDocker(consts.RedisVideoClientDB, t)
	defer cleanUpFunc()
	if err != nil {
		t.Fatal(err)
	}
	...
	cases := []struct {
	...
	}{
		{
			...
			op: func() (string, error) {
				for _, v := range videoList {
					if err = manager.CreateVideo(c, v); err != nil {
						return "", err
					}
				}
				return "", nil
		...
		},
		{
			...
			op: func() (string, error) {
				err = manager.CreateVideo(c, videoList[0])
				return "", err
			},
			wantErr: true,
		},
		{
			...
			op: func() (string, error) {
				video, err := manager.GetVideoByVideoId(c, videoList[0].ID)
				if err != nil {
					return "", err
				}
		...
		},
		{
			...
			op: func() (string, error) {
				video, err := manager.GetVideoListByAuthorId(c, videoList[0].AuthorId)
				if err != nil {
					return "", err
				}
				result, err := sonic.Marshal(video)
		...
		},
		{
			...
			op: func() (string, error) {
				videoList, err := manager.BatchGetVideoByVideoId(c, []int64{videoList[0].ID, videoList[1].ID})
				if err != nil {
					return "", err
				}
		...
		},
		{
			...
			op: func() (string, error) {
				err := manager.DeleteVideoById(c, videoList[0].ID)
				return "", err
			},
			wantErr:    false,
			wantResult: "",
